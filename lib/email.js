...
// Strip accidental surrounding quotes from env value; fallback to the verified custom domain
const FROM_EMAIL = (process.env.RESEND_FROM_EMAIL || 'Das Deutsche Haus <noreply@das-deutsche-haus.com>').replace(/^["']|["']$/g, '')
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'info@das-deutsche-haus.com'
const WHATSAPP_NUMBER = (process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '4915254196668').replace(/\D/g, '')
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
        ...
        <tr><td style="padding:32px 36px 12px;text-align:center;">
          <div style="display:inline-block;background:${BRAND.black};color:${BRAND.gold};padding:8px 16px;border-radius:50px;font-size:11px;font-weight:bold;letter-spacing:1px;">DAS DEUTSCHE HAUS</div>
        </td></tr>
...
export function tmplAdminNewLead({ leadType, lead }) {
  const labels = {
    ...
    contact_message: { ar: 'رسالة تواصل جديدة', emoji: '📩' },
  ...
  const detailsTable = `<table cellpadding="0" cellspacing="0" style="width:100%;background:#fafafa;border:1px solid #eee;border-radius:8px;padding:12px 16px;margin:12px 0;">
    ...
    ${infoRow('البلد / المدينة', lead.country)}
    ${infoRow('مستوى الألمانية', lead.germanLevel)}
    ${infoRow('المؤهل الدراسي', lead.education)}
...
export function tmplUserWelcome({ name, email, password, loginUrl }) {
  ...
  const body = `
    ...
    <ul style="margin:0 0 16px;padding-right:20px;line-height:2;">
      ...
      <li>طلب استشارات السفر والتقديم على Ausbildung</li>
    </ul>
...
export function tmplLeadConfirmation({ leadType, name }) {
  ...
  const body = `
    ...
    <div style="background:#f0f9ff;border-right:4px solid ${BRAND.red};padding:14px 18px;margin:16px 0;border-radius:8px;">
      ...
      <p style="margin:6px 0 0;font-size:13px;color:#555;">سيتواصل معك فريقنا في <strong>أقرب وقت</strong> لتأكيد التفاصيل وإرشادك للخطوة التالية.</p>
    </div>
    <p style="margin:16px 0 8px;font-weight:bold;">للتواصل المباشر:</p>
    <p style="margin:0 0 16px;line-height:2;">
      ${btn(`https://wa.me/${WHATSAPP_NUMBER}`, 'WhatsApp', '#25D366')}
      ${btn(APP_URL, 'زيارة الموقع', BRAND.black, BRAND.gold)}
    </p>
    <p style="color:#999;font-size:12px;margin-top:24px;">هذا إيميل تلقائي.</p>
  `
