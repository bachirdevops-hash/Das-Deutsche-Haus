          ...
          {jobs.map(j => (
            <Card key={j.id} className="card-hover">
              ...
              <CardContent className="p-6">
                ...
                <button onClick={() => selectJob(j)} className="w-full btn-primary py-2.5 rounded-xl font-bold flex items-center justify-center gap-2"><Send className="w-4 h-4" />{t.voc.apply}</button>
              </CardContent>
            </Card>
          ))}
        </div>
...
function AusbildungApplication({ lang, user, job }) {
  const ar = lang === 'ar'
  const L = {
