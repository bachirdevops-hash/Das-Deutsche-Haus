...
function Vocational({ t, lang, user }) {
  ...
  // 🎯 Select an Ausbildung and jump directly to the application section (#bewerbung)
  const selectJob = (j) => {
    setSelected(j)
            ...
            <Card key={j.id} className="card-hover">
              ...
              <CardContent className="p-6">
                ...
                <div className="flex gap-2">
                  <button onClick={() => selectJob(j)} className="flex-1 btn-primary py-2.5 rounded-xl font-bold flex items-center justify-center gap-2"><Send className="w-4 h-4" />{t.voc.apply}</button>
                  <a href={`/ausbildung/${j.slug || j.id}`} className="px-4 py-2.5 rounded-xl border-2 border-neutral-300 font-bold text-sm flex items-center justify-center hover:border-[#1A1A1A] hover:bg-neutral-50 transition whitespace-nowrap">{lang === 'ar' ? 'التفاصيل' : 'Details'}</a>
                </div>
              </CardContent>
            </Card>
          ))}
        </div>
        <AusbildungApplication lang={lang} user={user} job={selected} />
      </div>
    </section>
  )
}
